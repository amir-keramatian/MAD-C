#include "euclideanClusteringWrapper.h"
#include <omp.h>
...
int main(int argc, char* argv[]){
  ...
  for (int i = 0; i < numberOfThreads; i++){
    std::string basename(argv[i+1]);
    //std::ifstream ptCloudInFile((basename+".ptCloud").c_str());
    std::ifstream ptCloudInFile((basename).c_str());
    
    double x,y,z;
    while ( ptCloudInFile >> x >> y >> z){
      idxXYZ tuple; 
      tuple.x = x; tuple.y = y; tuple.z = z; tuple.clusterIDX =-1; tuple.index_ready = false;/*WARNING!*/
      threadData[i].data.push_back(tuple);
