...
envMap* volatile childrensMaps[NUMBER_OF_CHILDREN+1] = {NULL}; //just one more so that the compiler doesn't complain when there are is no children
...
int main(int argc, char* argv[]){
  ...
  for(int server = 0; server < NUMBER_OF_CHILDREN; server++){
    ...
    pthread_create(&tcpPid[server], NULL, startServer, &tcpThreadData[server]);
  }
  /*  
  ...
  */
  ...
  pthread_create(&tcpPid[NUMBER_OF_CHILDREN], NULL, startClient, &tcpThreadData[NUMBER_OF_CHILDREN]);
#endif
